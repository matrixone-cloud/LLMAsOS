...
from web_agent.utils.dom_helper import get_element_outer_html
from web_agent.utils.dom_mutation_observer import subscribe
from web_agent.utils.dom_mutation_observer import unsubscribe
...
async def do_entertext(page: Page, selector: str, text_to_enter: str, use_keyboard_fill: bool=True):
    ...
    try:
        ...
        if use_keyboard_fill:
            await elem.focus()#用于在浏览器页面中聚焦指定的 DOM 元素
            # 等待100ms确保完全获得焦点，模拟用户行为，减少被判断为机器人行为风险，确保页面的响应
            await asyncio.sleep(0.1)
            await press_key_combination("Control+A")#选择文本框内全部文本
            await asyncio.sleep(0.1)
            await press_key_combination("Backspace")#清除已有文本
            await asyncio.sleep(0.1)
            logger.debug(f"Focused element with selector {selector} to enter text")
            #add a 100ms delay
            await page.keyboard.type(text_to_enter, delay=1)#输入待搜索内容
        else:
