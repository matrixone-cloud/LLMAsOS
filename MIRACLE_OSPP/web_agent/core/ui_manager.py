...
from web_agent.config import PROJECT_SOURCE_ROOT
...
class UIManager:
                ...
                if message["from"] == "user":
                   ...
                   #choose chich message types to be shown depending on UI setting
                    if self.overlay_show_details == False:  # noqa: E712
                        ...
                        if message["message_type"] not in (MessageType.PLAN.value,  MessageType.QUESTION.value , MessageType.ANSWER.value,  MessageType.INFO.value, MessageType.STEP.value , MessageType.ACTION.value):
                            continue
    ...
    def new_cloud_message(self, message: str, type:MessageType=MessageType.ClOUD):
        self.conversation_history.append({"from":"cloud", "message":message, "message_type":type.value})
        print(f"Adding cloud message: {message}")
