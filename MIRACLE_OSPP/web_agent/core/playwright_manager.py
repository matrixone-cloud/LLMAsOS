...
from web_agent.core.notification_manager import NotificationManager
from web_agent.core.ui_manager import UIManager
from web_agent.utils.dom_mutation_observer import dom_mutation_change_detected
from web_agent.utils.dom_mutation_observer import handle_navigation_for_mutation_observer
from web_agent.utils.js_helper import beautify_plan_message
from web_agent.utils.js_helper import escape_js_message
...
class PlaywrightManager:
    ...
    async def overlay_state_handler(self, is_collapsed: bool):
        '''
        Args:
            is_collapsed: bool representing the collapsed state.

        Returns:
            获取当前页面实例
            更新 UI 管理器中的覆盖状态
            如果覆盖状态未折叠，更新覆盖聊天历史记录
        '''
    ...
    async def show_steps_state_handler(self, show_details: bool):
        '''
        Args:
            show_details: bool representing the show details.

        Returns:
            获取当前页面实例。
            更新 UI 管理器中的覆盖显示详细步骤状态。
        '''
        page = await self.get_current_page()
    ...
    async def notify_user(self, message: str, message_type: MessageType = MessageType.STEP):
        ...
        if message_type == MessageType.PLAN:
            ...
            message = "To planner: " +message

        safe_message = escape_js_message(message)

        if message_type ==MessageType.ACTION:
            self.ui_manager.new_cloud_message(safe_message, message_type)
        else:
        ...
        if self.ui_manager.overlay_show_details == True:  # noqa: E712
            if message_type not in (MessageType.PLAN,  MessageType.QUESTION , MessageType.ANSWER,  MessageType.INFO, MessageType.STEP, MessageType.ACTION):
                return
        ...
        self.notification_manager.notify(message, message_type.value)   #useless

    async def notify_user_cloud(self, message: str, message_type: MessageType = MessageType.STEP):
        if message.startswith(":"):
            message = message[1:]
        if message.endswith(","):
            message = message[:-1]
        ...
        safe_message_type = escape_js_message(message_type.value)
        try:
            js_code = f"addSystemMessage({safe_message}, is_awaiting_user_response=true, message_type={safe_message_type});"
            page = await self.get_current_page()
            await page.evaluate(js_code)
        except Exception as e:
            logger.error(
