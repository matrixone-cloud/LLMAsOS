...
def final_reply_callback_planner_agent(message:str, message_type:MessageType = MessageType.STEP): # type: ignore
        ...
        loop.run_until_complete(browser_manager.notify_user(message, message_type=message_type))#运行完成此步界面上就显示了
        return False, None  # required to ensure the agent communication flow continues

def final_reply_callback_chat_response(message:str, message_type:MessageType = MessageType.STEP):
    browser_manager = PlaywrightManager(browser_type='chromium', headless=False)
    loop = asyncio.get_event_loop()
    loop.run_until_complete(browser_manager.notify_user_cloud(message, message_type=message_type))  # 运行完成此步界面上就显示了
    return False, None  # required to ensure the agent communication flow continues
